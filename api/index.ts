...
// ── Inline engine (simplified for serverless) ──────────────
const SHARE_PAYOUT_CENTS = 100;

function lockRequired(side: string, type: string, priceCents: number | null, qty: number, feeBps: number) {
...
function computeTakerFee(execPrice: number, qty: number, feeBps: number) {
...
app.get("/me", { preHandler: [authenticate] }, async (req) => {
  return prisma.user.findUniqueOrThrow({ where: { id: (req as any).user.sub }, select: { id: true, email: true, role: true, createdAt: true } });
});

app.get("/wallet", { preHandler: [authenticate] }, async (req) => {
  const w = await prisma.wallet.findUnique({ where: { userId: (req as any).user.sub } });
...
app.get("/markets/:id/orders", { preHandler: [authenticate] }, async (req) => {
  const { id } = req.params as { id: string };
  return prisma.order.findMany({ where: { marketId: id, userId: (req as any).user.sub }, orderBy: { createdAt: "desc" }, take: 100 });
});

app.get("/markets/:id/position", { preHandler: [authenticate] }, async (req) => {
  const { id } = req.params as { id: string };
  const p = await prisma.position.findUnique({ where: { marketId_userId: { marketId: id, userId: (req as any).user.sub } } });
  return p ?? { yesShares: 0, avgCostCents: 0, realizedPnlCents: 0, lockedCents: 0 };
});

// ── Orders (serverless: DB-only matching, no in-memory book) ──
app.post("/markets/:id/orders", { preHandler: [authenticate] }, async (req, reply) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (status === "CANCELED" || status === "REJECTED") {
      const refund = required - remainingLock;
      // already decremented per-fill; just release leftover for unfilled
...
app.post("/markets/:id/orders/:orderId/cancel", { preHandler: [authenticate] }, async (req, reply) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (order.userId !== userId && (req as any).user.role !== "ADMIN") throw new Error("FORBIDDEN");
...
app.get("/admin/events", { preHandler: [requireAdmin] }, async (req) => {
  const { market_id, limit } = req.query as any;
...
app.post("/anchor-bets", { preHandler: [authenticate] }, async (req, reply) => {
  ...
  const ab = await prisma.anchorBet.create({ data: { creatorUserId: (req as any).user.sub, ...body } });
...
app.post("/anchor-bets/:id/side-bets", { preHandler: [authenticate] }, async (req, reply) => {
  ...
  const sb = await prisma.sideBet.create({ data: { anchorBetId: id, userId: (req as any).user.sub, ...body } });
...
// ── Helpers ────────────────────────────────────────────────
async function upsertPos(tx: any, marketId: string, userId: string, sharesDelta: number, priceCents: number, posLockDelta: number) {
